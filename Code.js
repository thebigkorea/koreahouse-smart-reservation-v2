 ...
 * 웹앱 진입점
 * - 기본: 스마트 예약관리 화면
 * - ?page=customer-manager : 고객관리 CRM
 * - ?page=sms-manager      : 문자관리
 * - ?action=getTodayReservationCount : ERP 오늘 예약 현황 JSON
 */
function doGet(e) {
  const page = (e && e.parameter && e.parameter.page) || "";
  const action = (e && e.parameter && e.parameter.action) || "";

  // 더큰코리아/한국의집 ERP 외부 연동용
  if (action === "getTodayReservationCount") {
    try {
      const data = getTodayReservationData();
      ...
      return ContentService
        .createTextOutput(JSON.stringify({
          ...
          depositWait: Number(data.depositWait || 0),
...
function setupKoreaHouseDB() {
  ...
  setupSheet_(ss, "Reservations", [
    "예약번호", "등록일시", "예약일", "예약시간", "고객명", "전화번호", "인원",
    "좌석", "행사구분", "메뉴", "예약금", "입금상태", "예약상태", "담당자",
    "요청사항", "예약경로", "예약확정문자", "전날안내문자", "방문완료",
    "방문완료시간", "수정일시"
  ]);

  setupSheet_(ss, "Customers", [
    "고객명", "전화번호", "첫예약일", "최근예약일", "예약횟수", "취소횟수",
    "노쇼횟수", "고객등급", "고객태그", "즐겨찾는메뉴", "알레르기", "메모",
    "최근수정일시"
  ]);

  setupSheet_(ss, "Seats", [
    "예약일", "예약시간", "좌석", "예약번호", "고객명", "상태"
  ]);

  setupSheet_(ss, "SMSHistory", [
    "발송일시", "예약번호", "고객명", "전화번호", "문자종류", "문자내용",
    "발송자", "발송결과", "Solapi Message ID", "비고", "재발송횟수"
  ]);

  setupSheet_(ss, "ReservationHistory", [
    "기록일시", "예약번호", "작업", "변경항목", "변경전", "변경후", "작업자"
  ]);

  setupSheet_(ss, "Settings", ["구분", "항목", "값", "비고"]);
  setupSheet_(ss, "Users", ["이름", "권한", "사용여부", "비고"]);
  setupSheet_(ss, "Logs", ["기록일시", "작업자", "작업", "상세내용"]);
...
function rebuildCustomerStatistics() {
  ...
  cusSheet.getRange(
    2,
    1,
    Math.max(cusSheet.getLastRow() - 1, 1),
    13
  ).clearContent();
